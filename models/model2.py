...
class multi_head_full_attns(nn.Module):

    def __init__(self, in_dim1, in_dim2, hidden, head, DEVICE):
        super(multi_head_full_attns, self).__init__()

        self.num_head = head
        self.hidden = hidden
        self.Q_mapping1 = nn.Linear(in_dim2, 1, bias=False)
        self.Q_mapping2 = nn.Linear(in_dim1, hidden * head, bias=False)
        self.K_mapping1 = nn.Linear(in_dim2, 1, bias=False)
        self.K_mapping2 = nn.Linear(in_dim1, hidden * head, bias=False)
    ...
    def forward(self, x):
        ...
        Q = self.Q_mapping2(torch.relu(self.Q_mapping1(x).squeeze(-1)))    # (B,N,hidden*head)
        Q = Q.view(B, attn_dim, self.num_head, self.hidden)    # (B,N,head,hidden)
        Q = Q.permute(2,0,1,3)    # (head, B, N, hidden)

        K = self.K_mapping2(torch.relu(self.K_mapping1(x).squeeze(-1)))    # (B,N,hidden*head)
        K = K.view(B, attn_dim, self.num_head, self.hidden)    # (B,N,head,hidden)
        K = K.permute(2,0,1,3)    # (head, B, N, hidden)

        A = torch.matmul(Q, self.W)    # (head, B, N, hidden)
        A = torch.einsum('hbij,hbjk->hbik', A, K.permute(0,1,3,2))    # return (head, B, attn_dim, attn_dim)
        A = F.softmax(A, -1)    # return (head, B, attn_dim, attn_dim)

        return A

class multihead_GCN(nn.Module):

    def __init__(self, adj, time_step, feature_dim, head, DEVICE):
        super(multihead_GCN, self).__init__()

        self.head = head
        self.attn_cal = multi_head_full_attns(in_dim1=feature_dim, in_dim2=time_step, hidden=32, head=head, DEVICE=DEVICE)
        self.W = nn.Parameter(torch.rand(feature_dim, feature_dim).float().to(DEVICE))
        self.adj = adj
        self.Wout = nn.Parameter(torch.rand(feature_dim, feature_dim).float().to(DEVICE))
    ...
    def forward(self, x):
        ...
        attn = self.attn_cal.forward(x.permute(0,1,3,2))    # (head,B,N,N)
        adj2 = attn * self.adj.unsqueeze(0).unsqueeze(0)     # (head,B,N,N)
        Attn_score = torch.mean(adj2, 0)    # (B,N,N)

        # we calculate different attntion score but the same values
        x_new = torch.matmul(x, self.W)    # return (B,N,T,F)

        # apply aggregation
        x_new = (torch.einsum('bij,bjkl->bikl', Attn_score, x_new))    # return (B,N,T,F)
        x_new = torch.matmul(x_new, self.Wout)    # return (B,N,T,F)
     
        return x_new, Attn_score
...
class AutoCorrelation(nn.Module):
    ...
    def __init__(self, device, adj, out_len, feature_dim, patch, input_len, topk, n_head):
        ...
        self.GCN_agg = multihead_GCN(adj=adj, time_step=out_len, feature_dim=feature_dim, head=n_head, DEVICE=device)
...
class Autoformer(nn.Module):
    def __init__(self, DEVICE, adj, configs):
        ...
        # recent traffic encoder
        self.cell3 = AutoCorrelation(device=DEVICE, adj=adj, out_len=self.pred_len, feature_dim=configs.d_model, patch=1, input_len=self.pred_len, topk=configs.topk, n_head=configs.n_heads)

        # decoder
        self.cell4 = AutoCorrelation(device=DEVICE, adj=adj, out_len=out_len_list[2], feature_dim=configs.d_model, patch=patch_list[2], input_len=out_len_list[1], topk=configs.topk, n_head=configs.n_heads)
        self.cell5 = AutoCorrelation(device=DEVICE, adj=adj, out_len=self.pred_len, feature_dim=configs.d_model, patch=patch_list[3], input_len=out_len_list[2], topk=configs.topk, n_head=configs.n_heads)
    ...
    def forward(self, x_enc):
        ...
        x_dec = x_enc[:, :, -self.pred_len:, :]    # (B,N,pred_len,F)
        ...
        # encode the most recent information
        x_dec_out, explain3= self.cell3.forward(x_dec, x_dec, x_dec)    # (B,N,pred_len,F)
        x_dec += x_dec_out

        # expand decoder input, (B,N,out_len_list[2],F)
        x_dec = torch.cat((x_dec, torch.zeros(B, N, x_enc.shape[2]-self.pred_len, self.d_model).to(x_dec.device)), 2)

        # cross correlation
        x_dec_out, explain4 = self.cell4.forward(x_dec, x_enc, x_enc)    # (B,N,out_len_list[2],F)
        x_dec = x_dec_out + x_dec[:,:,:x_dec_out.shape[2],:]
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, t):
        ...
        out, explains = self.A1(x_enc)    # (B,N,pred_len, F)
