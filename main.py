...
# define the model setting
if 'define configs' == 'define configs':
    ...
    parser.add_argument('--out_len', nargs='+', type=int, default=[12*24, 12*12, 12*4], help='model hyperparameter: output length of each cell')
    parser.add_argument('--patch_list', nargs='+', type=int, default=[12, 6, 3, 1], help='model hyperparameter: patch size of each cell')
...
# model.load_state_dict(torch.load('./trained_models/model_{}.pkl'.format(args.pred_len)))
...
train_data = data[:, :5*time_step_per_week]
test_data = data[:, 4*time_step_per_week:6*time_step_per_week]
mean_val = np.mean(train_data)
std_val = np.std(train_data)
train_loader = trainingset_construct(args=args, traffic_data=train_data, batch_val=2,\
...
for epoch in range(5):
    torch.save(model.state_dict(), './trained_models/model_{}.pkl'.format(args.pred_len))
    print('Current epoch', epoch)
    ma = eval(test_loader, model, device, args)
    train_loss = train(train_loader, model, optimizer, criterion, device, args)
