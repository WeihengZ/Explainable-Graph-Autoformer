...
parser.add_argument('--time_steps', type=int, default=12, help='length of the input time sequence, default is one week')
...
test_data = data[:, 14*time_step_per_week:16*time_step_per_week]
mean_val = np.mean(train_data)
std_val = np.std(train_data)
train_loader = trainingset_construct(args=args, traffic_data=train_data, batch_val=32,\
     num_data_limit=np.inf, Shuffle=True, mean=mean_val, std=std_val)
test_loader = trainingset_construct(args=args, traffic_data=test_data, batch_val=32,\
     num_data_limit=np.inf, Shuffle=False, mean=mean_val, std=std_val)

# perform training and testing
for epoch in range(5):
    torch.autograd.set_detect_anomaly(True)
    torch.save(model.state_dict(), r'./trained_models/model_push_accuracy_{}_epoch{}.pkl'.format(args.pred_len, epoch))
    print('Current epoch', epoch)
    train_loss = train(train_loader, model, optimizer, criterion, device, mean_val, std_val, update_step=1)
